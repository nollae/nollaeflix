...
const LabelCommon = styled.label`
    font-weight: 400;
    line-height: 1.5;
...
const EmailLabel = styled(LabelCommon)<{$isEmailFocus?:boolean}>`
...
const EmailInput = styled(InputCommon)<{$isEmailFocus?:boolean,}>`
    filter: opacity(${(props) => props.$isEmailFocus ? 100 : 0}%);
`;

const InputBorderCommon = styled.div`
...
const EmailInputBorder = styled(InputBorderCommon)<{$isFocus?:boolean, $isEmailError?:boolean, $isValid?:boolean}>`
...
const ErrorContainerCommon = styled.div`
    font-size: 0.8125rem;
    font-weight: 400;
    line-height: 1.53846;
    color: rgb(235, 57, 66);
...
const EmailErrorContainer = styled(ErrorContainerCommon)<{$isEmailError?:boolean}>`
    svg {
...
const PasswordLabel = styled(LabelCommon)<{$isPasswordFocus?:boolean}>`
...
const PasswordInput = styled(InputCommon)<{$isPasswordFocus?:boolean,}>`
    margin-right: 32px;
    filter: opacity(${(props) => props.$isPasswordFocus ? 100 : 0}%);
`;

const PasswordInputBorder = styled(InputBorderCommon)<{$isFocus?:boolean, $isPasswordError?:boolean, $isValid?:boolean}>`
    outline: ${(props) => props.$isFocus ? "rgb(255, 255, 255) solid 0.125rem" : "none"};
    outline-offset: ${(props) => props.$isFocus ? "0.125rem" : "none"};
    border-color: ${(props) => props.$isPasswordError ? "rgb(235, 57, 66)" : (props.$isValid) ? "rgb(43, 184, 113)" : "rgba(128, 128, 128, 0.7)"} ;
`;

const PasswordErrorContainer = styled(ErrorContainerCommon)<{$isPasswordError?:boolean}>`
    svg {
...
const PasswordShowHide = styled.button<{$isDisplay?:boolean, $isShow?:boolean}>`
...
function Login(){
    ...
    }, [login.email, setValue, trigger, setFocus]);
                ...
                <LoginContainer>
                    ...
                    <FormContainer onSubmit={handleSubmit(onValid)}>
                        ...
                        <PasswordWrapper >
                            ...
                            <PasswordShowHide onMouseDown={(event) => event.preventDefault()} onClick={handleShowHide} $isDisplay={isDisplay} $isShow={isShowPwShow} role="button" data-uia="icon-button" title="비밀번호 표시"type="button">
                            ...
                            <PasswordShowHide onMouseDown={(event) => event.preventDefault()} onClick={handleShowHide} $isDisplay={isDisplay} $isShow={isShowPwHide} role="button" data-uia="icon-button" title="비밀번호 숨기기" type="button">
                                <div aria-hidden="true">
                    ...
                    <FooterCotainer>
                        <SignWrapper>
                            Netflix 회원이 아닌가요? <SingLink href="/">지금 가입하세요</SingLink>.
                        </SignWrapper>
                        <ExplainWrapper>
                            <Explain>
